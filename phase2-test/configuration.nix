...
in
{
  ...
  config = mkMerge [
    ...
    {
      skarabox.hostname = "phase2-test";
      # Separated-key architecture (auto-detected from SOPS config):
      ...
      skarabox.hashedPasswordFile = config.sops.secrets."phase2-test/user/hashedPassword".path;
      ...
      sops.age = {
        # Separated-key mode: use secure runtime key for SOPS (default for new hosts)
        sshKeyPaths = [ "/persist/ssh/runtime_host_key" ];
      };

      sops.secrets."phase2-test/user/hashedPassword" = {
